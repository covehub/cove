...
SA_BENCH_DIR_PATTERN = re.compile(r"sa-bench_isl_(\d+)_osl_(\d+)")
...
def _workload_lengths_from_path(result_file: Path) -> tuple[int | None, int | None]:
    """Read ISL/OSL from the SA-Bench result directory name when JSON omits it."""
    match = SA_BENCH_DIR_PATTERN.fullmatch(result_file.parent.name)
    if not match:
        return None, None
...
def main(log_dir: Path) -> None:
    ...
    for result_file in result_files:
        ...
        if not config:
            isl, osl = _workload_lengths_from_path(result_file)
            config = {
                "model": data.get("model_id"),
                "isl": data.get("random_input_len") or isl,
                "osl": data.get("random_output_len") or osl,
            }
        ...
        runs.append({
            ...
            "total_input_tokens": data.get("total_input_tokens", data.get("total_input")),
            "total_output_tokens": data.get("total_output_tokens", data.get("total_output")),
        })
