...
class ArtifactDefinition:
    ...
    def materialized_hub_path(self, workflow_publisher: str) -> str:
        if self.hub_path.startswith("v1/"):
...
def workflow_with_generated_artifact_hub_paths(
    workflow: WorkflowDefinition,
    hub_paths: Mapping[str, str],
) -> WorkflowDefinition:
    artifacts: dict[str, ArtifactDefinition] = {}
    for artifact_name, artifact in workflow.artifacts.items():
        artifacts[artifact_name] = ArtifactDefinition(
            name=artifact.name,
            type=artifact.type,
            owner=artifact.owner,
            owner_domain=artifact.owner_domain,
            hub_path=hub_paths.get(artifact_name, artifact.hub_path),
            plaintext_hash=artifact.plaintext_hash,
            producer_node=artifact.producer_node,
            producer_service=artifact.producer_service,
        )

    normalized_data = copy.deepcopy(workflow.normalized_data)
    ...
    return WorkflowDefinition(
        ...
        platform_provider=workflow.platform_provider,
        platform_runtime=workflow.platform_runtime,
        owners=workflow.owners,
        ephemeral_keypairs=workflow.ephemeral_keypairs,
        artifacts=artifacts,
...
def _normalized_artifact_data(artifact: ArtifactDefinition) -> dict[str, Any]:
    ...
    if artifact.hub_path:
        payload["hub_path"] = artifact.hub_path
...
def _validate_artifacts(
    ...
    *,
    ...
    for artifact_name, raw_artifact in artifacts.items():
        ...
        if "hub_path" in artifact:
            errors.append(
                f"artifacts.{artifact_name}.hub_path is generated by cove compile and must not be authored"
            )
            continue
        ...
        if artifact_type == "dynamic":
            ...
            validated[artifact_name] = ArtifactDefinition(
                ...
                hub_path=f"runtime/{workflow_id}/artifacts/{artifact_name}/latest",
                plaintext_hash=None,
            )
            continue
